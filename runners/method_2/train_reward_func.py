...
def main():
    ...
    # read dataset
    from method_2.data import get_attribute_data
    ...
    logger = Experiment_Tracker(saved_dir='results',
                                expr_name=expr_name_prefix + f'reward-func-{cfg.dataset_name}',
                                project_name='relative-attribute-method-2', use_wandb=sys_args.use_wandb)
    cfg.reward_func.expr_id = logger.expr_name
    logger.redirect_output_to_logfile_as_well()
    logger.save_config(cfg)

    from method_2.trainer import Reward_Func_Trainer
    trainer = Reward_Func_Trainer(cfg, attr_training_data,
