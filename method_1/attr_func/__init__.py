...
def get_trained_model(load_from, default_cfg=None, return_optimizer=False):
    ...
    if default_cfg is not None:
        OmegaConf.update(default_cfg, 'attr_func',
                         loaded_cfg.attr_func,
                         force_add=True)
    ...
    attr_func = Attr_Func(loaded_cfg).to(device)
    attr_func.load_state_dict(model_params)

    # load optimizer
    optimizer = None
    if return_optimizer:
        raise NotImplementedError
    return attr_func, optimizer, loaded_cfg
