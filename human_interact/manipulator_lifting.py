...
class Manipulator_Lifting_Interact:
...
def main():
    ...
    target_attrs = Dict({
        'hardness': {
            'score': [0.6, 0.3],  # 0 to 0.9, step size: 0.06
            'epsilon': 0.06,  # low precision:  0.06, high precision: 0.03
        },
        'force': {
            'score': [0.85, 0.7],  # 0.35 to 0.95, step size: 0.05
            'epsilon': 0.05,  # low precision: 0.05, high precision: 0.01
        }
    })
    attr_names = list(target_attrs.keys())

    if algo == 'method_1':
        from method_1.human_interact.manipulator_lifting_agent import Manipulator_Lifting_Agent
        if use_language:
            agent = Manipulator_Lifting_Agent(reward_func_path='trained_models/manipulator_lifting/language/method_1/reward_func')
        else:
            agent = Manipulator_Lifting_Agent(reward_func_path='trained_models/manipulator_lifting/method_1/reward_func')
    elif algo == 'method_2':
        from method_2.human_interact.manipulator_lifting_agent import Manipulator_Lifting_Agent
        if use_language:
            agent = Manipulator_Lifting_Agent(reward_func_path='trained_models/manipulator_lifting/language/method_2/')
        else:
            agent = Manipulator_Lifting_Agent(reward_func_path='trained_models/manipulator_lifting/method_2/')
    else:
        raise NotImplementedError

    human_interact = Manipulator_Lifting_Interact(agent, render=False)
